  ...
  Future<ApiResponse> login(String login, String password, String? deviceId) async {
    try {
      ...
      try {
        ...
        print('\n\n=== DETAILED RESPONSE ANALYSIS FOR LOGIN: $login, PASSWORD: $password ===');
        ...
        if (responseJson['user'] != null) {
          ...
          if (responseJson['user']['employee'] != null) {
            ...
            if (responseJson['user']['employee']['pin'] != null) {
              ...
              print('\n!!! PIN CODE FOR ADMIN USER: ${responseJson['user']['employee']['pin']} !!!');
            } else {
              print('\nPIN field not found in employee object');
              // Шукаємо всі можливі поля, які можуть містити PIN
              final possiblePinFields = ['pin', 'employee_pin', 'user_pin', 'badge_pin', 'security_pin', 'password', 'pwd'];
              for (final field in possiblePinFields) {
                if (responseJson['user']['employee'][field] != null) {
                  print('\nFound possible PIN in employee.$field: ${responseJson['user']['employee'][field]}');
                }
              }
            }
          } else {
            print('\nNo employee object found in user');
            // Шукаємо поле pin в корені об'єкта user
            final possiblePinFields = ['pin', 'employee_pin', 'user_pin', 'badge_pin', 'security_pin', 'password', 'pwd'];
            for (final field in possiblePinFields) {
              if (responseJson['user'][field] != null) {
                print('\nFound possible PIN in user.$field: ${responseJson['user'][field]}');
              }
            }
          }
        }
